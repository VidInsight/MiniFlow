...
class ExecutionOutputOrchestrator(BaseOrchestrator):
    ...
    @with_orchestration_errors('get_execution_output_by_id')
    @with_session
    def get_by_id(self, session, *, record_id: str, user_id: str, include_relationships: bool = False, exclude_fields: List[str] = None) -> Optional[Dict[str, Any]]:
        """
        Get execution output by ID with execution access control.
        User must have access to the parent execution.
        """
        record_id = self._validate_execution_output_exist(session, record_id)
        user_id = self._validate_user_exist(session, user_id)
        
        # Get execution output to find execution_id
        execution_output = self.execution_output_crud._get_by_id(session, record_id)
        ...
        if not has_access:
            self._raise_permission_denied('access', user_id, execution_output.execution_id, 'execution')
    ...
    @with_orchestration_errors('get_all_execution_outputs')
    @with_session
    def get_all(self, session, *, user_id: str, execution_id: str = None, skip: int = 0, limit: int = 100, order_by: Optional[str] = None, order_desc: bool = False, include_deleted: bool = False, exclude_fields: List[str] = None, **filters) -> List[Dict[str, Any]]:
        """
        Get all execution outputs accessible to user via execution permissions.
        ...
        query = select(self.execution_output_crud.model)
        query = self._build_parent_junction_query(
            query,
            user_id,
            self.user_execution_role_crud,
            Execution,
            self.execution_output_crud.model,
        ...
        if execution_id:
            query = query.where(self.execution_output_crud.model.execution_id == execution_id)
        
        # Apply is_deleted filter
        if not include_deleted:
            query = query.where(self.execution_output_crud.model.is_deleted == False)
        
        # Apply additional filters
        for key, value in filters.items():
            if hasattr(self.execution_output_crud.model, key):
                query = query.where(getattr(self.execution_output_crud.model, key) == value)
        
        # Apply ordering
        if order_by and hasattr(self.execution_output_crud.model, order_by):
            order_column = getattr(self.execution_output_crud.model, order_by)
