...
@router.get("/{script_id}", response_model=APIResponse[Dict[str, Any]])
@with_api_error_handling(operation="get_script")
async def get_script(script_id: str, include_relationships: bool = Query(False), exclude_fields: Optional[str] = Query(None), operations: ScriptOperations = Depends(get_script_operations), correlation_id: str = Depends(get_current_correlation_id)):
    exclude_list = exclude_fields.split(',') if exclude_fields else None
    result = await operations.get_script_record(script_id, include_relationships, exclude_list)
    return APIResponse(data=result, message="Script retrieved", correlation_id=correlation_id)

@router.put("/{script_id}", response_model=APIResponse[Dict[str, Any]])
...
@router.delete("/{script_id}", response_model=APIResponse[Dict[str, Any]])
...
@router.get("/{script_id}/test-stats", response_model=APIResponse[Dict[str, Any]])
@with_api_error_handling(operation="get_test_stats")
async def get_test_stats(script_id: str, operations: ScriptOperations = Depends(get_script_operations), correlation_id: str = Depends(get_current_correlation_id)):
    result = await operations.get_test_stats(script_id)
    return APIResponse(data=result, message="Test Stats", correlation_id=correlation_id)

@router.get("/{script_id}/performance-stats", response_model=APIResponse[Dict[str, Any]])
@with_api_error_handling(operation="get_performance_stats")
async def get_performace_stats(script_id: str, operations: ScriptOperations = Depends(get_script_operations), correlation_id: str = Depends(get_current_correlation_id)):
    result = await operations.get_performance_stats(script_id)
    return APIResponse(data=result, message="Performance Stats", correlation_id=correlation_id)