...
@router.get("/{execution_input_id}", response_model=Dict[str, Any])
@with_api_error_handling(operation="get_execution_input")
async def get_execution_input(execution_input_id: str, operations: ExecutionInputOperations = Depends(get_execution_input_operations), correlation_id: str = Depends(get_current_correlation_id)):
    result = await operations.get_execution_input_record(execution_input_id)
