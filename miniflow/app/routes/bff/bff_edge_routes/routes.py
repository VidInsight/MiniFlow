...
async def get_edge(edge_id: str, operations: EdgeOperations = Depends(get_edge_operations), correlation_id: str = Depends(get_current_correlation_id)):
    result = await operations.get_edge_record(edge_id)
    return APIResponse(data=result, message="Edge retrieved", correlation_id=correlation_id)

@router.put("/{edge_id}", response_model=Dict[str, Any])
...
@router.delete("/{edge_id}", response_model=Dict[str, Any])
